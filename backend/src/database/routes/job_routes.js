...
router.patch("/job-status/:jobId", async (req, res) => {
  ...
  try {
    const updatedJob = await job_queries.updateJobStatus(
      parseInt(jobId, 10),
      status
    );
    if (updatedJob) {
...
router.get("/all-jobs", async (req, res) => {
  try {
    ...
    const distanceMiles = parsePositiveNumber(filters.distanceMiles);

    if (filters.distanceMiles !== undefined && distanceMiles === null) {
      return res.status(400).json({
    ...
    const currentUserId =
      req.session?.user_id ||
      (filters.currentUserId ? parseInt(filters.currentUserId, 10) : null);
    ...
    return res.json({
      ...
      appliedDistanceFilter: Boolean(distanceMiles),
      distanceUnit: distanceMiles ? "miles" : null,
    });
  } catch (error) {
