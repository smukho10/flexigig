...
// POST /api/reviews/worker-to-employer
// body: { reviewer_id, reviewee_id, job_id, rating?, review_text? }
router.post("/reviews/worker-to-employer", async (req, res) => {
  try {
    const reviewer_id = req.body.reviewer_id ? parseInt(req.body.reviewer_id, 10) : null;
    const reviewee_id = req.body.reviewee_id ? parseInt(req.body.reviewee_id, 10) : null;
    const job_id      = req.body.job_id      ? parseInt(req.body.job_id,      10) : null;
    ...
    // Validate: reviewer is the worker with ACCEPTED application on this job,
    // and reviewee is the employer who owns the job
    const validation = await db.query(
      `
      SELECT ga.application_id
      FROM gig_applications ga
    ...
    if (validation.rows.length === 0) {
      return res.status(403).json({
        message: "Worker can only review an employer for a job they were accepted on.",
      });
    }
    ...
    console.error("Error creating worker-to-employer review:", err);
    if (err && err.code === "23505") {
      return res.status(400).json({ message: "You have already reviewed this employer for this job." });
    }
    return res.status(500).json({ message: "Internal server error" });
  }
});
